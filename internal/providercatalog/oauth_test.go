...
func TestOAuthProviders(t *testing.T) {
	...
	if len(providers) != 5 {
		t.Fatalf("OAuthProviders() = %d, want 5 (openrouter, xai, kimi-code, huggingface, chatgpt)", len(providers))
	}
	...
	kimi, ok := byID["kimi-code"]
	if !ok || kimi.OAuthMintsKey || !kimi.OAuthDeviceFlow || !kimi.OAuthDeviceOnly {
		t.Fatalf("kimi-code oauth flags wrong: %+v", kimi)
	}
}
...
func TestNonOAuthProvidersNotFlagged(t *testing.T) {
	for _, d := range All() {
		switch d.ID {
		case "openrouter", "xai", "kimi-code", "huggingface", "chatgpt":
			continue
		}
		if d.OAuth {
