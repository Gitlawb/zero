...
// IsRuntimeIdentityHeader reports whether a header key is a runtime identity
// header (such as Kimi's X-Msh-* headers) that should be generated dynamically
// rather than persisted in saved configuration profiles.
func IsRuntimeIdentityHeader(key string) bool {
	return strings.HasPrefix(strings.ToLower(key), "x-msh-")
}
