...
func TestReclaimStaleLock(t *testing.T) {
	dir := t.TempDir()
	lockPath := filepath.Join(dir, "tokens.lock")

	// A genuinely stale lock (old mtime) is reclaimed and removed.
	if err := os.WriteFile(lockPath, []byte("crashed-holder"), 0o600); err != nil {
		t.Fatal(err)
	}
	old := time.Now().Add(-2 * fileLockStaleAfter)
	if err := os.Chtimes(lockPath, old, old); err != nil {
		t.Fatal(err)
	}
	if ok, err := reclaimStaleLock(lockPath, "tok-a", fileLockStaleAfter); err != nil || !ok {
	...
	if ok, err := reclaimStaleLock(lockPath, "tok-b", fileLockStaleAfter); err != nil || ok {
	...
	if ok, err := reclaimStaleLock(lockPath, "tok-c", fileLockStaleAfter); err != nil || ok {
