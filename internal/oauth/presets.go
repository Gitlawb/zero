...
// isCanonicalKimiHost reports whether urlStr is an explicitly approved Kimi
// OAuth/API endpoint. Only the hosts this package's preset and managed coding
// path talk to are allowed, and only over https: arbitrary *.kimi.com /
// *.moonshot.cn subdomains and cleartext (or non-http) schemes must not
// receive the persistent device identity.
