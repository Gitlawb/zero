...
func TestProviderManagerCaseOnlyRenameSynchronizesLiveSession(t *testing.T) {
	m := managerTestModel(t)
	...
	if next.providerName != "OpenGateway" || next.providerProfile.Name != "OpenGateway" {
		t.Fatalf("live session did not follow case-only rename: name=%q profile=%q", next.providerName, next.providerProfile.Name)
	}
	...
	if !strings.Contains(next.providerWizard.manageStatus, "Press Enter") {
		t.Fatalf("active edit omitted apply guidance: %q", next.providerWizard.manageStatus)
	}
	if persisted := readManagerConfig(t, next.userConfigPath); persisted.Providers[0].Name != "OpenGateway" {
		t.Fatalf("persisted provider was not renamed: %+v", persisted.Providers)
	}
}
...
func TestProviderManagerEditDoesNotRetargetCaseVariantProjectSession(t *testing.T) {
	...
	if next.providerName != "WORK" || next.providerProfile.Name != "WORK" {
		t.Fatalf("project session was retargeted: name=%q profile=%q", next.providerName, next.providerProfile.Name)
	}
	if got := os.Getenv(config.ActiveProviderEnv); got != "WORK" {
		t.Fatalf("%s = %q, want WORK", config.ActiveProviderEnv, got)
	}
	if strings.Contains(next.providerWizard.manageStatus, "Press Enter") {
		t.Fatalf("unrelated project session got apply guidance: %q", next.providerWizard.manageStatus)
	}
	if next.providerWizard.manageActiveName != "WORK" {
		t.Fatalf("active manager row = %q, want exact project profile", next.providerWizard.manageActiveName)
	}
	if persistedConfig := readManagerConfig(t, configPath); len(persistedConfig.Providers) != 1 || persistedConfig.Providers[0].Name != "Work" {
		t.Fatalf("persisted edit targeted wrong row: %+v", persistedConfig.Providers)
	}
}
