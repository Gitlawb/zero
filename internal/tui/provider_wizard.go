...
// oauthAttemptSeq issues OAuth attempt IDs that stay unique for the lifetime
// of the process. A per-wizard counter is not enough: the wizard is discarded
// wholesale on Esc and rebuilt on the next open, so its counter restarts and
// the second wizard reissues ID 1 for the same provider. A late result from
// the first, canceled attempt then satisfies oauthResultMatches and can
// persist a credential into, or advance, a wizard that never requested it.
var oauthAttemptSeq atomic.Int64

// nextOAuthAttemptID returns the next process-wide attempt ID. Never zero, so
// a zero value still reads as "no attempt".
func nextOAuthAttemptID() int {
	return int(oauthAttemptSeq.Add(1))
}
...
func (wizard *providerWizardState) beginOAuthAttempt(device bool) int {
	...
	wizard.oauthAttemptID = nextOAuthAttemptID()
...
func (m model) handleProviderWizardKey(msg tea.KeyMsg) (model, tea.Cmd) {
	...
	if m.providerWizard.manage && !m.providerWizard.managerStep() && keyIs(msg, tea.KeyEsc) {
		...
		if wizard.oauthPending {
			wizard.oauthPending = false
			// Burn the ID so a result from the attempt being abandoned here
			// cannot match once oauthPending goes true again.
			wizard.oauthAttemptID = nextOAuthAttemptID()
		}
		wizard.cancelDeviceLogin()
...
func stripRuntimeIdentityHeaders(profile config.ProviderProfile) config.ProviderProfile {
	...
	for key := range out.CustomHeaders {
		if providercatalog.IsRuntimeIdentityHeader(key) {
