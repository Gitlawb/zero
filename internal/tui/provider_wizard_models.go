...
var providerWizardCuratedModels = map[string][]providerWizardModel{
	"ollama": {
...
func providerWizardModelOptions(provider providercatalog.Descriptor) []providerWizardModel {
	if models, ok := providerWizardCuratedModels[provider.ID]; ok {
		return providerWizardDedupeModels(provider.DefaultModel, models)
	}
	models := providerWizardRegistryModels(provider)
	if len(models) > 0 {
		return providerWizardDedupeModels(provider.DefaultModel, models)
	}
	return providerWizardDedupeModels(provider.DefaultModel, nil)
}

func providerWizardRegistryModels(provider providercatalog.Descriptor) []providerWizardModel {
	...
	for _, entry := range registry.List(modelregistry.ListOptions{}) {
		...
		models = append(models, providerWizardModel{ID: entry.ID, Description: entry.DisplayName, Meta: meta})
		if len(models) >= 8 {
			break
		}
	}
...
func providerWizardDedupeModels(defaultModel string, models []providerWizardModel) []providerWizardModel {
	result := []providerWizardModel{}
	seen := map[string]bool{}
	add := func(model providerWizardModel) {
	...
	add(providerWizardModel{ID: defaultModel, Description: "catalog default"})
