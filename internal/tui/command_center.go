...
func (m model) switchProviderModel(providerName, modelID string) (model, string, bool, tea.Cmd, error) {
	...
	if strings.TrimSpace(m.userConfigPath) != "" {
		...
		case owner.UserBacked:
			if cfg, err := config.SetActiveProviderModel(m.userConfigPath, owner.PersistedName, target.Model); err != nil {
				persistErr = err
	...
	if persisted {
		...
		status += " · not saved (" + redaction.RedactString(persistErr.Error(), redaction.Options{}) + ")"
	}
	status += persistNote
	if warn := m.visionDropWarning(); warn != "" {
		status += "\n" + warn
