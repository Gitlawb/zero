...
// TestProviderWizardManagerEscCancelsDeviceLoginPoll is the manager-mode half
// of TestProviderWizardEscCancelsDeviceLoginPoll. handleProviderWizardKey has
// a separate Esc branch for manage mode that cancels the poll but keeps the
// wizard open, so the non-manager test above never exercised it. The poll must
// still be canceled: leaving it running means finishing the login in the
// browser later silently saves a credential the user backed out of.
func TestProviderWizardManagerEscCancelsDeviceLoginPoll(t *testing.T) {
	m := managerTestModel(t)
	m.providerWizard = m.newProviderWizard()
	m.providerWizard.manage = true
	m.providerWizard.selectedMethod = 0
	next, _ := m.advanceProviderWizard()
	m = selectWizardOAuthProvider(t, next, "xai")
	m.providerWizard.manage = true
	if m.providerWizard.managerStep() {
		t.Fatal("setup: expected an OAuth step, not a manager list/edit step")
	}
	...
	// Manager mode keeps the wizard; only the login is abandoned.
	if escaped.providerWizard == nil {
		t.Fatal("Esc in manager mode should keep the wizard open")
	}
	if escaped.providerWizard.oauthPending {
		t.Fatal("Esc should clear the pending login")
	}
	if escaped.providerWizard.deviceLoginCancel != nil {
		t.Fatal("Esc should clear the stored cancel func")
	}
	...
	if !errors.Is(msg.err, context.Canceled) {
		t.Fatalf("poll error = %v, want context.Canceled (manager-mode Esc should have canceled the background poll)", msg.err)
	}
}

// TestProviderWizardAttemptIDsSurviveWizardClose pins that an attempt ID from
// a wizard the user abandoned cannot be reissued by the next one. The wizard
// is discarded wholesale on Esc, so a counter living on it restarts, and a
// late result from the canceled attempt would then satisfy oauthResultMatches
// for a brand new attempt against the same provider, persisting a credential
// the second wizard never asked for.
func TestProviderWizardAttemptIDsSurviveWizardClose(t *testing.T) {
	m := mouseTestModel()
	m.providerWizard = m.newProviderWizard()
	m.providerWizard.selectedMethod = 0
	next, _ := m.advanceProviderWizard()
	...
	firstProvider, firstAttempt := beginTestOAuthAttempt(m.providerWizard, true)
	if firstAttempt == 0 {
		t.Fatal("first attempt should get a non-zero id")
	}

	// Abandon the wizard the way Esc does, then open a fresh one and start the
	// same provider again.
	...
	next, _ = m.advanceProviderWizard()
	m = selectWizardOAuthProvider(t, next, "xai")
	secondProvider, secondAttempt := beginTestOAuthAttempt(m.providerWizard, true)

	if firstProvider != secondProvider {
		t.Fatalf("setup: expected the same provider both times, got %q then %q", firstProvider, secondProvider)
	}
	if secondAttempt == firstAttempt {
		t.Fatalf("a reopened wizard reused attempt id %d; a late result from the abandoned attempt would match the new one", firstAttempt)
	}
	// The stale attempt must not be accepted by the live wizard.
	if m.providerWizard.oauthResultMatches(firstProvider, firstAttempt) {
		t.Fatal("a result from the abandoned attempt was accepted by the reopened wizard")
	}
	if !m.providerWizard.oauthResultMatches(secondProvider, secondAttempt) {
		t.Fatal("the live attempt should be accepted")
	}
}
...
// TestProviderWizardProfileOmitsKimiHeadersOnCustomEndpoint is the
// custom-endpoint half of the test above, which only covers the canonical
// Kimi base URL. X-Msh-* carries a persistent device id and the machine
// hostname, so baking it into a profile aimed at an arbitrary staging or proxy
// host would send both somewhere Kimi does not own.
func TestProviderWizardProfileOmitsKimiHeadersOnCustomEndpoint(t *testing.T) {
	isolateKimiDeviceIDStorage(t)
	m := mouseTestModel()
	m.providerWizard = m.newProviderWizard()
	m.providerWizard.selectedMethod = 0
	...
	provider := m.providerWizard.currentProvider()
	custom := "https://kimi-proxy.example.test/v1"
	if sameProviderBaseURL(custom, provider.DefaultBaseURL) {
		t.Fatalf("setup: %q must differ from the canonical endpoint", custom)
	}

	profile := providerWizardProfile(provider, provider.DefaultModel, "", custom, "")

	for key := range profile.CustomHeaders {
		if strings.HasPrefix(strings.ToLower(key), "x-msh-") {
			t.Fatalf("device identity header %q leaked to a custom endpoint: %#v", key, profile.CustomHeaders)
		}
	}
	if profile.BaseURL != custom {
		t.Fatalf("profile BaseURL = %q, want the custom endpoint %q", profile.BaseURL, custom)
	}
}
...
func TestModelQuitCancelsSetupDeviceLogin(t *testing.T) {
	m := mouseTestModel()
	canceled := false
	m.setup.deviceLoginCancel = func() {
		canceled = true
	}
	_, cmd := m.quit()
	if !canceled {
		t.Fatal("m.quit() did not invoke setup.deviceLoginCancel")
	}
	if cmd == nil {
		t.Fatal("m.quit() should return tea.Quit cmd")
	}
}
