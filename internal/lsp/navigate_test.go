...
func TestNavigateUnknownOp(t *testing.T) {
	...
	_, _, ok, err := m.Navigate(context.Background(), NavRequest{Op: "bogus", Path: "x.go"})
	if err == nil {
...
func TestNavigateUnsupportedExtensionDegrades(t *testing.T) {
	...
	m := NewManager(t.TempDir())
	_, _, ok, err := m.Navigate(context.Background(), NavRequest{Op: NavDefinition, Path: "notes.unknownext", Line: 1, Character: 1})
	if err != nil {
