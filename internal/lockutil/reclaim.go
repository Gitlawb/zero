...
// a lost race it returns false. A non-nil error means a live holder's lock
// could not be restored (both the no-replace restore and its copy fallback
// failed), so lockPath may be missing; callers must fail closed instead of
// re-acquiring. The sidelined file is removed on every restore failure: once
// the restore has failed it has no protocol function (release only consults
// the lock path), so keeping it would only leak files.
