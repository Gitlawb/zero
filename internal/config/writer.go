...
func validateCatalogProviderLogin(cfg FileConfig, catalogID string) error {
	...
	if catalogProvider {
		if _, owned, err := catalogProviderOwner(cfg.Providers, descriptor.ID); err != nil {
			return err
		} else if owned {
			return nil
		}
...
func EnsureCatalogProvider(path string, catalogID string) (EnsuredProvider, error) {
	...
	written, err := runProviderProfileOperation(path, true, false, func(op *providerProfileOperation) error {
		if owner, owned, err := catalogProviderOwner(op.config.Providers, descriptor.ID); err != nil {
			return err
		} else if owned {
			result = EnsuredProvider{Name: owner.Name, Active: op.config.ActiveProvider}
			op.publish = false
			return nil
		}
